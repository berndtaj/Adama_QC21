####################################################################
# Analysis with overlapping (Soma & OLink) proteins
# Date:      08-09-2021
# Author:    Adama J.Berndt
####################################################################
# Purpose: To repeat final iteration of Olink analysis using 
#          only the set of overlapping Soma/OLink proteins.
####################################################################
...
for (i in 1:nrow(Olink.age)) {
  soma.dat$AGE..YRS[soma.dat$PatientID == Olink.age$PatientID[i]] <- 
    Olink.age$AGE..YRS[i]
}
...
# Select targets
STargets <- soma.match.list$Target
OTargets <- soma.match.list$OLink.Assay
OTargets <- str_replace_all(soma.match.list$OLink.Assay, "-", ".")
OTargets2 <- str_replace_all(OTargets, " ", ".")
OTargets3 <- str_replace_all(OTargets2, "/", ".")



OTargets3[357] <- "CGA" #Splits into "CGA,LHB" independent readings, same protein different panels
OTargets3[228] <- "LHB" #Splits into "CGA,LHB" independent readings, same protein different panels
OTargets3[116] <- "TSHB" #Splits into "CGA,TSHB" independent readings, same protein different panels
OTargets3[155] <- "IL12"##Becomes IL12 because IL.1B2 is not in olink dataset


OTargets3[125] <- "CCL3"  # Overwrite Redundant protein 
OTargets3[177] <- "uPA"  # Overwrite Redundant protein 
OTargets3[182] <- "CXCL1"  # Overwrite Redundant protein
OTargets3[185] <- "OPG" # Overwrite Redundant protein
OTargets3[230] <- "Beta.NGF" # Overwrite Redundant protein
OTargets3[254] <- "BOC" # Overwrite Redundant protein
OTargets3[278] <- "MCP.1" # Overwrite Redundant protein
OTargets3[311] <- "IL6" # Overwrite Redundant protein
OTargets3[328] <-"PGF" # Overwrite Redundant protein

OTargets4 <- OTargets3[- 317] # Delete Second FUT3/FUT5

sum((OTargets4 %in% colnames(olink.dat)) == FALSE)

# Select data
ctl.dat <- cbind(olink.dat[olink.dat$Group == 'NC',c(1:11)], 
                 olink.dat[olink.dat$Group == 'NC',OTargets4])
ND.dat <- cbind(olink.dat[olink.dat$Group != 'NC',c(1:11)], 
                olink.dat[olink.dat$Group != 'NC',OTargets4])
...
# Generate data frame
model356.df <- data.frame(Protein = character(356), Uniprot = character(356),
                        Age.coef = numeric(356), Age.pval = numeric(356),
                        Age.FDR = numeric(356),
                        Male.coef = numeric(356), Male.pval = numeric(356),
                        Male.FDR = numeric(356),
                        #PC1.coef = numeric(356), PC1.pval = numeric(356),
                        #PC1.FDR = numeric(356)
                        )

# Run loop and apply linear model to all proteins
for (i in 1:356) {
  model356.df$Protein[i] <- names(ctl.dat)[i+11]
  formula_i <- paste(model356.df$Protein[i], "~AGE..YRS+Sex", sep = "")
  model_i <- lm(formula_i, ctl.dat)
  model356.df$Age.coef[i] <- summary(model_i)$coefficients[2,1]
  model356.df$Age.pval[i] <- summary(model_i)$coefficients[2,4]
  model356.df$Male.coef[i] <- summary(model_i)$coefficients[3,1]
  model356.df$Male.pval[i] <- summary(model_i)$coefficients[3,4]
}

# Adjust p-values
model356.df$Age.FDR <- p.adjust(model356.df$Age.pval, method = 'BH')
model356.df$Male.FDR <- p.adjust(model356.df$Male.pval, method = 'BH')
model356.df$PC1.FDR <- p.adjust(model356.df$PC1.pval, method = 'BH')

# Summary
sum(model356.df$Age.FDR < 0.05) # 100 age-associated proteins
model356.df <- model356.df %>% arrange(Age.FDR)
View(model356.df)
setwd("C:/Users/nezne/Desktop/Aging Project/Aging Project/OLink/Olink Analysis p = 356")
write.csv(model356.df, 
          file = "Age Associated Linear model results 08-10-2021.csv",
          row.names = FALSE)



sum(sig.proteins %in% model356.df$Protein)
####################################################################

# 2) MODEL SELECTION WITH ONLY 100 TOP PROTEINS:
...
Y <- ctl.dat$AGE..YRS
X <- model.matrix(AGE..YRS~., data = ctl.dat[,c(4,5,7,idx.sig)])[, -1]  # proteins,sex,PC1
count.matrix <- data.frame(Param = colnames(X), Non.Zero = numeric(64))
for (i in 1:1000) {
  set.seed(i)
...
ggplot(count.matrix) +
  ...
  labs(title = "Repeated Elastic Net with 10-fold CV
       (100% samples, 62 proteins+Sex,PC1)",
       x = "Times with non-zero coefficient", y = "Parameters")
# Summary
sum(count.matrix$Non.Zero == 1000) # 35
sum(count.matrix$Non.Zero >= 900)  # 39

# Second, using 90% samples:
count.matrix_0.9 <- data.frame(Param = colnames(X), Non.Zero = numeric(64))
for (i in 1:1000) {
  set.seed(i)
...
ggplot(count.matrix_0.9) +
  geom_histogram(aes(x = Non.Zero), bins = 20) +
  labs(title = "Repeated Elastic Net with 10-fold CV
       (90% samples, 62 proteins+Sex,PC1)",
       x = "Times with non-zero coefficient", y = "Parameters")

# Lastly, using 70% samples:
count.matrix_0.7 <- data.frame(Param = colnames(X), Non.Zero = numeric(64))
for (i in 1:1000) {
  set.seed(i)
...
ggplot(count.matrix_0.7) +
  geom_histogram(aes(x = Non.Zero), bins = 20) +
  labs(title = "Repeated Elastic Net with 10-fold CV
       (70% samples, 62 proteins+Sex,PC1)",
       x = "Times with non-zero coefficient", y = "Parameters")
# Save results
write.csv(count.matrix %>% arrange(1000-Non.Zero), 
          "Stability selection (100-pct data, 62proteins).csv",
          row.names = FALSE)
write.csv(count.matrix_0.9 %>% arrange(1000-Non.Zero), 
          "Stability selection (90-pct data, 62proteins).csv",
          row.names = FALSE)
write.csv(count.matrix_0.7 %>% arrange(1000-Non.Zero), 
          "Stability selection (70-pct data, 62proteins).csv",
          row.names = FALSE)

# Understand results
sum(count.matrix_0.7$Non.Zero > 500) # 32
sum(count.matrix_0.7$Param[count.matrix_0.7$Non.Zero > 500] %in%
      count.matrix$Param[count.matrix$Non.Zero == 1000])   # All 32 are here
...
# TIMD3, IL.18.BPa, UNC5H3 left out

####################################################################

# 3) MODEL SELECTION WITH EXHAUSTIVE SEARCH:

# Now, let's perform exhaustive search for the top 35 proteins
ss_35 <- count.matrix$Param[count.matrix$Non.Zero == 1000]
idx3 <- match(ss_35, names(ctl.dat))
...
all.models <- data.frame(Var.Num = 1:35,
                         Formula = character(35),
                         R.sqrd = numeric(35),
                         Cp = numeric(35),
                         BIC = numeric(35),
                         AIC = numeric(35),
...
write.csv(all.models.exh, "All models exhaustive search 2-6-21.csv",
          row.names = FALSE)
...
RMSE.exh <- matrix(0, nrow = 1000, ncol = 35)
R.sqr.exh <- matrix(0, nrow = 1000, ncol = 35)
Train.cor.exh <- matrix(0, nrow = 1000, ncol = 35)
Test.cor.exh <- matrix(0, nrow = 1000, ncol = 35)
for (i in 1:35){
...
which.min(apply(RMSE.exh, 2, mean))       # 35
which.max(apply(R.sqr.exh, 2, mean))      # 35
which.max(apply(Train.cor.exh, 2, mean))  # 35
which.max(apply(Test.cor.exh, 2, mean))   # 21 
apply(Test.cor.exh, 2, mean)              # Average cor = 0.8538908

# Save
write.csv(RMSE.exh, "RMSE for exh search models 2-6-21.csv",
          row.names = FALSE)
write.csv(R.sqr.exh, "R.sqr for exh search models 2-6-21.csv",
          row.names = FALSE)
write.csv(Train.cor.exh, "Train set correlations 2-6-21.csv",
          row.names = FALSE)
write.csv(Test.cor.exh, "Test set correlations 2-6-21.csv",
          row.names = FALSE)
...
# Predict
final_model <- lm(all.models.exh$Formula[21], ctl.dat)
